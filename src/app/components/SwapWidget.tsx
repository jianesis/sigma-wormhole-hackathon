...
const SwapWidget: React.FC<SwapWidgetProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
        ...
        if (window.MayanSwap && isOpen) {
          ...
          window.MayanSwap.init("swap_widget", {
            ...
            colors: {
              ...
              primaryGradient:
                "linear-gradient(180deg, #2F855A 0%, #1B4332 100%)",
    ...
    const loadScript = () => {
      if (customElements.get("wcm-button")) {
        console.log(
          "Custom elements already defined, proceeding with widget initialization"
        );
        initWidget();
        return;
      }
      ...
      if (isOpen && !scriptRef.current) {
        ...
        script.integrity =
          "sha256-xg2EIE9pWR7nYXqX9IE+d2Lajrd34w0aKbuJHFH2+aw=";
  ...
  return (
    ...
    >
      ...
      >
        <div id="swap_widget" className="w-full h-full" />
      </div>
    </div>
  );
};
